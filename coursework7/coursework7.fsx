...
let pathWf (path: FileSystem.Path) : bool =
    ...
    if fs.name = "" then
        ...
        | children ->
            if hasDuplicates children then
                ...
                let filteredItems =
                    children |> List.filter (fun el -> fsTreeWf el)
...
(*
   ...
   generators (meaning that p and fs are well-formed).
*)
let createCreates (p: Path) (fs: FsTree) : bool = show (create p fs) |> List.contains p
...
let showShowsEverything (fs: FsTree) : bool =
    isEmpty (
        (show fs)
        |> List.fold (fun accFs p -> delete p accFs) fs
    )
